...
Banks & Mokel (2023) @bank23-multiple reflect on advances in MSE for human trafficking and emphasize the importance of moving beyond naive assumptions when applying MSE in policy contexts (Banks & Mokel, 2023).

@zhan21-estimating  Zhang & Larsen (2021) discuss using MSE for policy advocacy and service design in the context of human trafficking, highlighting its value but also its limitations when data structures evolve over time (Zhang & Larsen, 2021).
...
# **Research Project on MSE and Causal Inference in Modern Slavery Interventions**

## Outline

### **Overview**

This research project investigates whether **Multiple Systems Estimation (MSE)** methods can reliably assess changes in the size of hidden populations—such as victims of modern slavery—over time and under intervention. The project involves a **theoretical evaluation**, **simulation modeling**, and exploration of **causal inference methods**, particularly when interventions (e.g., by the **International Justice Mission, IJM**) affect the structure and behavior of the data-generating mechanisms (e.g., referral lists from NGOs, police, immigration).

* * *

**1. Evaluating MSE for Time-Based Comparisons**
------------------------------------------------

### **Context and Challenge**

The central challenge is whether MSE can be used to compare population sizes between time points (e.g., pre- and post-intervention), especially when:

* **List definitions change** over time (e.g., due to agency coordination improvements),
    
* **Referral processes evolve**, affecting who appears in the lists,
    
* **List overlaps increase** not due to true prevalence change but due to better inter-agency cooperation.
    

### **Technical Concern: SUTVA Violation**

If a treatment affects not just individual victimization but also how victims are recorded (e.g., through list overlap), this violates the **Stable Unit Treatment Value Assumption (SUTVA)**—a foundational assumption in causal inference.

> _"If part of the intervention is to increase coordination across agencies... increase overlap, you get a smaller population, but that’s not because you reduced prevalence—it’s a measurement artifact."_

* * *

**2. Using MSE for Causal Inference: Quasi-Experimental Design**
----------------------------------------------------------------

### **Experimental Setup**

* The IJM rolled out anti-slavery interventions in multiple countries (e.g., **Romania**, **Poland**) at different times.
    
* The researchers explore whether this staggered timing can enable a **Difference-in-Differences (Diff-in-Diff)** approach to estimate the **causal impact of the intervention**.
    

### **Design Limitation**

* There is **no pure control group**; all countries are treated eventually.
    
* Estimating the counterfactual—what would have happened without treatment—requires the **parallel trends assumption**, i.e., untreated countries must mirror the trajectory treated countries would have followed without intervention.
    

> _"The assumption is that whatever that growth rate is, it’s the same in the treated and untreated groups... That’s the parallel slope assumption."_

### **Extension Reference**

* The project references **Susan Athey’s** work on **non-parametric Diff-in-Diff**, which could relax linearity and parallel slope constraints.
    

* * *

**3. Simulation Design to Test MSE Robustness**
-----------------------------------------------

### **Objectives**

* Identify whether **changes in list overlap**—not prevalence—can distort MSE-derived population estimates.
    
* Evaluate whether MSE can still detect real changes when both prevalence and overlap shift.
    

### **Key Scenarios**

* **Strawman Simulation:** Prevalence remains fixed, but overlap between lists increases due to agency coordination → MSE erroneously infers reduced population.
    
* **Compound Simulation:** Prevalence decreases due to real treatment effect, but increased overlap exaggerates this reduction → MSE overstates the treatment’s effectiveness.
    

> _“We could show simulations where true prevalence falls, but because of overlap changes, it looks like nothing changed—or even that prevalence increased.”_

### **Modeling Details (To Be Developed)**
...
* Metrics: Estimate bias, false positives/negatives in population change detection.
    

* * *

**4. Technical Considerations on List Use**
-------------------------------------------

### **Dependence Among Lists**

* MSE assumes **independence** between lists. If this is violated (e.g., police automatically refer cases to immigration), estimates become biased.
...
* Researchers debated whether to:
    ...
    * Explicitly model the dependence using interactions and criteria like **AIC/BIC**.
...
This project interrogates whether MSE can be reliably used for **both measurement and inference** under dynamic intervention settings. It challenges conventional assumptions in MSE, explores new territory at the **intersection of causal inference and hidden population estimation**, and proposes:

* A robust **simulation framework** to expose limitations of standard MSE,
    
* Consideration of new designs (e.g., staggered rollouts, list correction models),
    
* Practical recommendations for **multi-agency field interventions**.
...
* **Literature Review:** Investigate overlaps in MSE and causal inference literature, particularly Bernard’s and Athey’s work.
    
* **Simulation Prototyping:** Formalize simulation parameters, develop preliminary models.
    
* **Methodological Paper:** Draft a conceptual paper on MSE under intervention and SUTVA violation, even without empirical data.
    

> _"There’s a huge MSE literature… and a huge causal inference literature… but I’m unaware of any intersection. That’s good news for us then."_

This project could be among the first to formally explore that intersection.
...
Each individual is represented by a binary vector (e.g., `[1, 0, 1]` means captured on Lists $A$ and $C$ e.g. the first and third lists).
